...
async def session_start(payload: SessionStartPayload, state: AppState = Depends(get_state)):
	"""Start a recording session. Creates session directory, video recording, frames.csv, IMU CSV."""
	async with state.session_lock:
		if state.session_id is not None:
			return {"detail": "Session already running", "session_id": state.session_id}
		sid = (payload.session_id or "").strip()
		if not sid:
			sid = time.strftime("%Y%m%d_%H%M%S")
		base = state.session_base_dir(sid)
				...
				json.dumps(
					{
						"session_id": sid,
						"t_start": t_start,
						"imu": {"mode": state.active_mode, "rate": state.active_rate},
		...
		state.imu_csv_fh = open(imu_path, "a", encoding="utf-8", newline="\n")
		if imu_new:
			state.imu_csv_fh.write("t,imu_timestamp,imu_sample_index,ax,ay,az,gx,gy,gz,mx,my,mz\n")
			state.imu_csv_fh.flush()
		state.session_id = sid
		state.session_dir = base
		return {"detail": "Session started", "session_id": sid, "dir": str(base)}


@router.post("/session/stop")
async def session_stop(state: AppState = Depends(get_state)):
	...
	async with state.session_lock:
		...
		try:
			if state.imu_csv_fh:
				state.imu_csv_fh.close()
		except Exception:
			pass
		state.imu_csv_fh = None
		t_stop = time.time()
		try:
			if state.session_dir:
				path = state.session_dir / "session.json"
				if path.exists():
		...
		try:
			if state.session_dir:
				p = state.session_dir / "frames.csv"
				if p.exists():
					video_backend = (state.video.name() or "video").strip()
					out = []
...
async def session_status(state: AppState = Depends(get_state)):
	return {"session_id": state.session_id, "dir": str(state.session_dir) if state.session_dir else None}


@router.get("/sessions/{session_id}/video")
async def get_session_video(session_id: str, state: AppState = Depends(get_state)):
	"""Serve a session video file for playback. Prefers video.mp4; may mux from video.h264."""
	base = state.session_base_dir(session_id)
	mp4 = base / "video.mp4"
	if mp4.exists():
...
async def debug_status(state: AppState = Depends(get_state)):
	"""Quick diagnostics: IMU flow, detection enabled, jump worker, queue depth."""
	q_size = None
	q_max = None
	try:
		if state.jump_sample_queue is not None:
			q_size = state.jump_sample_queue.qsize()
			q_max = getattr(state.jump_sample_queue, "_maxsize", None)
	except Exception:
		pass
	try:
		if state.count_clip_jobs_pending:
			state.dbg["clip_jobs_pending"] = int(state.count_clip_jobs_pending())
	except Exception:
		pass
	try:
		if state.count_clip_jobs_done:
			state.dbg["clip_jobs_done"] = int(state.count_clip_jobs_done())
	except Exception:
		pass
	try:
		if state.count_clip_jobs_failed:
			state.dbg["clip_jobs_failed"] = int(state.count_clip_jobs_failed())
	except Exception:
		pass
	try:
		if state.read_last_clip_job_error:
			state.dbg["clip_last_failed"] = state.read_last_clip_job_error()
	except Exception:
		pass
	return {
		"detection_enabled": bool(state.jump_detection_enabled),
